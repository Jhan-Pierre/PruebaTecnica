...
VALUES ('DNI', '75471704', 'Jhan Muñoz', 'M', 1, 1, 1857),
       ('DNI', '87654321', 'Ana Gomez', 'F', 1, 1, 1858);
GO

CREATE or ALTER PROCEDURE sp_ListarTrabajadores
    @SexoFiltro VARCHAR(1) = NULL 
AS
BEGIN
    SET NOCOUNT ON;
    ...
    FROM 
        dbo.Trabajadores t
    LEFT JOIN 
        dbo.Departamento dep ON t.IdDepartamento = dep.Id
    LEFT JOIN 
        dbo.Provincia p ON t.IdProvincia = p.Id
    LEFT JOIN 
        dbo.Distrito d ON t.IdDistrito = d.Id
    WHERE
        (@SexoFiltro IS NULL OR @SexoFiltro = '' OR t.Sexo = @SexoFiltro)
    ORDER BY
        t.Id DESC;
...
CREATE or ALTER PROCEDURE sp_CrearTrabajador
...
CREATE OR ALTER PROCEDURE sp_ObtenerTrabajadorPorId
    @Id INT
AS
BEGIN
    SELECT 
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.Trabajadores WHERE Id = @Id)
    BEGIN
        RETURN; 
    END
